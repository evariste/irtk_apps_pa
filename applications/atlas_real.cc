...
void usage()
{
  cerr << "Usage: atlas_real [output] <input1..inputN> <options>\n" << endl;
...
void normalise(irtkRealImage &input, irtkRealImage &output)
{
  int j;
  double mean, std;
  irtkRealPixel *ptr1;
...
int main(int argc, char **argv)
{
  ...
  irtkRealPixel tempMin, tempMax;
  input.GetMinMax(&tempMin, &tempMax);
  cout << "Min " << tempMin << " Max " << tempMax << endl;

  ptr1 = input.GetPointerToVoxels();
  ptr2 = output.GetPointerToVoxels();
  for (j = 0; j < n; j++) {
    *ptr1 = scale * *ptr2 / total_weight;
    ptr1++;
    ptr2++;
  }
  cerr << " done\n";
